...
public class MediaController extends FrameLayout {
    ...
    private TextView mEndTime, mCurrentTime, mTimeDiver;
    ...
    private OnSeekBarChangeListener mSeekListener = new OnSeekBarChangeListener() {
        ...
        public void onProgressChanged(SeekBar bar, int progress, boolean fromuser) {
            ...
            if (!isShowTime) {
                android.util.Log.e(">>tag", "" + mEndTime);
                if (mEndTime != null) {
                ...
                if (mTimeDiver != null) {
                    mTimeDiver.setVisibility(GONE);
                }
            } else {
                if (mTimeDiver != null) {
                    mTimeDiver.setVisibility(VISIBLE);
                }
                if (mEndTime != null) {
...
//        }
    ...
    private void initControllerView(View v) {
        ...
        change = (ImageView) v.findViewById(getResources().getIdentifier("full_play", "id", mContext.getPackageName()));
        ...
        mTimeDiver = (TextView) v.findViewById(getResources().getIdentifier("text_diver", "id", mContext.getPackageName()));
//        if (mFileName != null)
...
//            mFileName.setText(mTitle);
    ...
    public void show(int timeout) {
        if (!mShowing && mAnchor != null && mAnchor.getWindowToken() != null) {
            ...
            if (mFromXml) {
                ...
                setWindowLayoutType();
                mWindow.showAtLocation(mAnchor, Gravity.NO_GRAVITY, anchorRect.left, anchorRect.bottom - dip2px(mContext, 73));
            }
            mShowing = true;
    ...
    private void doPauseResume() {
        if (mPlayer.isPlaying()) {
            ...
            if (playStateChangeListener != null) {
                playStateChangeListener.isPause(true);
            }
        } else {
            mPlayer.start();
            if (playStateChangeListener != null) {
                playStateChangeListener.isPause(false);
            }
        }
        updatePausePlay();
    }
    ...
    public interface FullPlayInterface {
        void change(boolean isFull);
    }

    FullPlayInterface fullPlay;

    public void setFullPlay(FullPlayInterface fullPlay) {
    ...
    public void setIsFull(boolean isFull) {
        android.util.Log.e("tag", ">>>>>>>>>isFull>" + isFull);
        if (isFull) {
            change.setImageResource(getResources().getIdentifier("change_little", "drawable", mContext.getPackageName()));
        } else {
            change.setImageResource(getResources().getIdentifier("video_full_icon", "drawable", mContext.getPackageName()));
        }
    }
    ...
    public interface PlayStateChangeListener {
        void isPause(boolean isPause);
    }

    boolean isShowTime = true;

    public void isShowTime(boolean isShowTime) {
